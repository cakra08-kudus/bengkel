...
class PerbaikanDetail extends StatefulWidget {
  // PerbaikanDetail({Key? key}) : super(key: key);
  final String idPerbaikan;
  PerbaikanDetail({required this.idPerbaikan});

  @override
  State<PerbaikanDetail> createState() => _PerbaikanDetailState();
}

class _PerbaikanDetailState extends State<PerbaikanDetail> {
  Dio dio = Dio();
  ...
  tampil() async {
    var form = FormData.fromMap({
      "id_service": widget.idPerbaikan,
    });

    final respon = await dio.post(
        "https://bengkel.famenarakudus.com/api/detailService",
        data: form);
    ...
    if (respon.statusCode == 200) {
      ...
      print(data);
      setState(() {
        selectedDate = DateTime.parse(data['tgl_service'] + ' 00:00:00');
        selectedPilihan = data['pilihan_service'];
        selectedKendaraan = data['id_kendaraan_pelanggan'];
        selectedJasa = data['id_jasa'];
        pembawaKendaraan.text = data['pembawa_kendaraan'];
        kilometerKendaraan.text = data['kilometer'];
        keluhanKendaraan.text = data['keluhan'];
      });
    }
  }
  ...
  ubah() async {
    ...
    final res = await dio
        .post("https://bengkel.famenarakudus.com/api/editService", data: form);
